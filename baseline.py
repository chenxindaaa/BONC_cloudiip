...
def line_detect_possible_demo(image, center):
    res = {}
    ...
    for line in lines:
        ...
        cv.line(src, (x1 + center[0], center[1] - y1), (x2 + center[0],  center[1] - y2), (255, 0, 0), 1)
        #t = cv.waitKey(10000)
...
def get_center(mask):
    ...
    for i, contour in enumerate(contours):
        ...
        cx = w / 2
        cy = h / 2
        cv.circle(src, (np.int(x + cx), np.int(y + cy)), 1, (255, 0, 0))
...
# 均值迁移
blur = cv.pyrMeanShiftFiltering(src, 10, 16)
cv.imshow('blur', blur)
